...
def miq(V,
    ...
    # An example of computing sharp feature using igl and convert to (f_id, v_in_f_id) format
    ...
    # V_in_f_id = np.repeat(np.arange(3), len(F))[unique_idx][sharp]
    # V_in_f_id = (V_in_f_id + 1) % 3

    # SE = np.stack([F_id, V_in_f_id], -1)
